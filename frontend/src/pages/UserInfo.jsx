...
export default function ClassSetup() {
  ...
  const handleContinue = () => {
    const filled = classes.filter((c) => c.className.trim() && c.professorName.trim());

    if (filled.length === 0) {
      setError("Add at least one class with a name and professor before continuing.");
      return;
    }
      ...
      <div className="nt-inner" style={{ position: "relative", zIndex: 1, padding: "20px 32px 100px" }}>
        ...
        <motion.div initial="hidden" animate="show" style={{ textAlign: "center", marginBottom: 36 }}>
          ...
          <motion.p variants={fadeUp} custom={2} style={{ marginTop: 14, color: "rgba(245,241,232,0.6)", fontSize: 15, lineHeight: 1.6, maxWidth: 520, marginLeft: "auto", marginRight: "auto" }}>
            Add up to {MAX_CLASSES} classes so NoteTaker knows how to file your lectures.
            ...
            {classes.map((c, i) => (
              ...
              >
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
                  <div style={{ fontWeight: 800, fontSize: 14, color: "rgba(245,241,232,0.5)" }}>
                    CLASS {i + 1}
                  </div>
                  {classes.length > 1 && (
                    <span
                      ...
                      style={{ fontSize: 13, color: "rgba(245,241,232,0.4)", fontWeight: 700 }}
                    >
                      Remove
                    </span>
                  )}
                ...
                <div className="nt-field-row" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 12 }}>
                  <div>
                ...
                <div style={{ marginBottom: 12 }}>
                  <label style={{ fontSize: 12, color: "rgba(245,241,232,0.5)", fontWeight: 700, display: "block", marginBottom: 6 }}>
                    Usual lecture day & time
                  </label>
                  <input
                    className="nt-input"
                    value={c.lectureTime}
                    onChange={(e) => updateClass(c.id, "lectureTime", e.target.value)}
        ...
        {error && (
          <div style={{ marginTop: 14, color: "#ff8a8a", fontSize: 13, textAlign: "center" }}>
            {error}
          </div>
        )}
